...
uint8_t btld_FlashNext_32(uint32_t data,uint32_t* index){
    ...
    if(HAL_FLASH_Program( FLASH_TYPEPROGRAM_FAST , flash_ptr,(uint64_t) data) == HAL_OK)
    {
...
uint8_t btld_SaveChecksum(void){
	...
	returnedERR = HAL_FLASH_Program(TYPEPROGRAM_WORD,CRC_ADDRESS, (uint64_t)calculatedCrc);
